...
/// Evaluate multiple expressions and return their linear-combination
pub fn evaluate_lc<F: Field, B: Basis>(
    expression: &[Expression<F>],
    coefficients: &[F],
    size: usize,
    rot_scale: i32,
    ...
    parallelize(&mut values, |values, start| {
        for (i, value) in values.iter_mut().enumerate() {
            ...
            for (coeff, expr) in coefficients.iter().zip(expression.iter()) {
                *value += expr.evaluate(
                    &|scalar| scalar,
                    &|_| panic!("virtual selectors are removed during optimization"),
                    &|query| {
